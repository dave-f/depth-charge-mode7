...
const FRAME_ENTRY = 0x7115;            // JMP frame in the jump table
...
const h2 = cpu.debugWrite.add((addr, val) => {
    if (addr === 0x7121) flg = val;                 // frmflg written by frame
    if (addr === 0x7120 && val > 0 && cur) cur.evt = val;   // objevt bumped by walker
    return false;
});
if (dropEvery > 0) {
