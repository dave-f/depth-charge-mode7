...
// or to "MC" when the CPU is in the PLOT code (&6E00-&7BFF, including the
...
const [, wlo, whi] = s.readMemory(0x6E12, 3);
const walk = wlo | (whi << 8);
const cpu = s._machine.processor;
const evHook = cpu.debugWrite.add((addr, val) => {
    if (addr === 0x6E20 && val > 0) { curEvt = val; if (onlyEvt && val >= onlyEvt && !frameOn) { frameOn = true; framesCounted++; } }
    return false;
});
const hook = cpu.debugInstruction.add((pc) => {
    ...
    if (pc >= 0x6E00 && pc < 0x7c00) { mc += d; return false; }
